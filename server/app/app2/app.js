...
mongoClient.connect(url, (err, db) => {
  if (err) {
    ...
    app.get("/checkPassengerRequest/:email",(req,res)=>{
      // Route to check if passenger's request is in progress so that she/he will not make another duplicate request
      const email = { email: req.params.email }
      rideService.findOne(email, (err, result) => {
        if (result != null) {
          ...
          console.log("Request already in progress! "+JSON.stringify(responseToClient))
          res.status(200).send(JSON.stringify(responseToClient));
        } else {
          res.status(404).send();
        }
...
app.listen(PORT, () => {
  console.log("server is listening on port 3000 bitch!");
});
