...
public class RideInProgress_RecyclerAdapter extends RecyclerView.Adapter<RideInProgress_RecyclerAdapter.RequestViewHolder>{
    ...
    public void LogRequest(final int i){
        AlertDialog alertDialog = new AlertDialog.Builder(context).create();
        alertDialog.setTitle("Log Ride?");
        alertDialog.setMessage("Press Yes to log this ride to the database.\nPress outside the dialog to cancel");
        alertDialog.setButton(AlertDialog.BUTTON_NEUTRAL, "Yes",
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        HashMap<String, String> logData = new HashMap<>();
                        Call<Void> call = retrofitInterface.executeLogRide(logData);
                        logData.put("email", email[i]);
                        logData.put("destination", destination[i]);
                        logData.put("gpsCordinates", location[i]);
                        logData.put("pickuptime", pickup[i]);// Possibly add more data here, we will see
                        call.enqueue(new Callback<Void>() {
                            @Override
                            public void onResponse(Call<Void> call, Response<Void> response) {
                                if (response.code() == 200) {
                                    Toast.makeText(context, "Ride logged successfully", Toast.LENGTH_LONG).show();
                                } else if (response.code() == 400) {
                                    Toast.makeText(context, "Error occurred in logging ride", Toast.LENGTH_LONG).show();
                                }
